...
const TeacherMessages = () => {
  ...
  if (!lesson || lesson.teacherMessages.length === 0) return null;

  const messagesList = (
    <div className="w-full px-3 py-3 space-y-4">
      {lesson.teacherMessages.map((msg) => (
...
const PromptExamples = () => {
  ...
  if (!lesson || lesson.prompts.length === 0) return null;
...
const Divider = ({
  ...
  return (
    <div
      ...
      style={{
        height: 6,
        backgroundColor: hovered ? colors.accent + "30" : colors.border,
      }}
      ...
      const onMove = (ev: MouseEvent) => {
        ...
        if (dragging.current === "top") {
          setTopRatio(
            Math.max(
              0.1,
              Math.min(startTopRatio.current + dr, 1 - bottomRatio - 0.15)
            )
          );
        } else {
          setBottomRatio(
            Math.max(
              0.1,
              Math.min(startBottomRatio.current - dr, 1 - topRatio - 0.15)
            )
          );
        }
      };
